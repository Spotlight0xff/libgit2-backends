...
int main(int argc, char** argv) {
  ...
  std::string path = "test_repo";
  std::string file = "test_repo.db";
  int result = 0;
  int error = 0;
  ...
  // create fixtur
  git_libgit2_init();
  result = git_repository_init(&m_repo, path.c_str(), false);
  result = git_refdb_new(&ref_db, m_repo);
  result = git_refdb_backend_sqlite(&ref_backend, m_repo, file.c_str());
