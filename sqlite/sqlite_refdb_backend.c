// https://github.com/libgit2/libgit2-backends/blob/584fb0a0afebf24bb5488896bdd8323ee3faf69a/sqlite/sqlite_refdb_backend.c
// modified to work with current libgit2 version
#include <assert.h>
#include <string.h>
#include <git2.h>
#include <git2/tag.h>
...
{
  char buffer[256];
  //sprintf_s(buffer, 256, "Reference not found: %s", name);
  // replaced on linux with:
  snprintf(buffer, 256, "Reference not found: %s", name);
  giterr_set_str(GITERR_REFERENCE, buffer);
...
typedef struct {
  ...
  git_pool pool;
  git_vector loose;
  sqlite_refdb_backend *parentBackend;
  size_t loose_pos;
} sqlite_refdb_iter;
    ...
    git_reference **out, git_reference_iterator *_iter)
{
  ...
  sqlite_refdb_iter *iter = (sqlite_refdb_iter *)_iter;
  sqlite_refdb_backend *backend = iter->parentBackend;
    ...
    git_reference_iterator **out, git_refdb_backend *_backend, const char *glob)
{
  ...
  iter = (sqlite_refdb_iter*) calloc(1, sizeof(sqlite_refdb_iter));
    ...
    int force)
{
  if (!force) {
    ...
    if (exists) {
      char msg[1024];
      //sprintf_s(msg, 1024, "Failed to write reference '%s': a reference "
      //"with that name already exists.", new_ref);
      // replaced on linux with:
      snprintf(msg, 1024, "Failed to write reference '%s': a reference "
          "with that name already exists.", new_ref);

      giterr_set_str(GITERR_REFERENCE, msg);
      return GIT_EEXISTS;
    }
  }
    ...
    const char *old_target)
{
  ...
  error = reference_path_available(backend, git_reference_name(ref), NULL, force);
  if (error < 0)
    return error;

  error = SQLITE_ERROR;

  if (sqlite3_bind_text(backend->st_write, 1, git_reference_name(ref), -1, SQLITE_TRANSIENT) == SQLITE_OK) {
    if (git_reference_type(ref) == GIT_REF_OID) {
      char oid[GIT_OID_HEXSZ + 1];
      git_oid_nfmt(oid, sizeof(oid), git_reference_target(ref));

      error = sqlite3_bind_text(backend->st_write, 2, (char *)oid, -1, SQLITE_TRANSIENT);
    } else if (git_reference_type(ref) == GIT_REF_SYMBOLIC) {
      char *symbolic_ref = (char*)malloc(strlen(GIT_SYMREF)+strlen(git_reference_symbolic_target(ref))+1);

      //strcpy_s(symbolic_ref, sizeof(symbolic_ref), GIT_SYMREF);
      //strcat_s(symbolic_ref, sizeof(symbolic_ref), git_reference_symbolic_target(ref));
      strncpy(symbolic_ref, GIT_SYMREF, sizeof(symbolic_ref));
      strncat(symbolic_ref, git_reference_symbolic_target(ref), strlen(git_reference_symbolic_target(ref))); //, sizeof(symbolic_ref));
...
static int sqlite_refdb_backend__delete(git_refdb_backend *_backend, const char *name, const git_oid *old, const char *old_target)
{
  sqlite_refdb_backend *backend = (sqlite_refdb_backend *)_backend;
  int error;
    ...
    const char *message)
{
  ...
  if ((error = sqlite_refdb_backend__delete(_backend, old_name, 0, 0)) < 0) {
    git_reference_free(old);
    return error;
  }

  git_reference_rename(&vNew, old, new_name, 0, 0);//git_reference__set_name(old, new_name);
  if (!vNew) {
    git_reference_free(old);
    return -1;
  }

  if ((error = sqlite_refdb_backend__write(_backend, vNew, force, who, message, 0, 0)) > 0) {
    git_reference_free(vNew);
    return error;
  }
    ...
    const char *sqlite_db)
{
  ...
  sqlite3_bind_text(backend->st_write, 1, "HEAD", -1, SQLITE_TRANSIENT);
  sqlite3_bind_text(backend->st_write, 2, "ref: refs/heads/master", -1, SQLITE_TRANSIENT);
  sqlite3_step(backend->st_write);
  sqlite3_reset(backend->st_write);
